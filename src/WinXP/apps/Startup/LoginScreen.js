...
const LoginScreen = ({ onLogin }) => {
  ...
  const handlePasswordSubmit = e => {
    ...
    if (password.toLowerCase() === activeAccount.password) {
      handleLogin(activeAccount);
    } else {
      setLoginError(true);
      setPassword('');
    }
  ...
  const handleLogin = account => {
    ...
    // 2.5 saniye sonra desktop'a geç (orijinal gibi)
    setTimeout(() => {
      onLogin(account);
    }, 1500);
  };
  ...
  const handleOutsideClick = e => {
    if (!e.target.closest('.login-screen__account')) {
  ...
  return (
    <LoginContainer onClick={handleOutsideClick}>
      <LoginTop />
      <LoginCenter>
        <LoginInstructions>
          <img src={windowsXpLogoWhiteTextBgCropped} alt="Windows logo" />
          ...
          {accounts.map(account => (
            <LoginAccount
              ...
              className={`login-screen__account ${
                ...
                isLoggingIn && activeAccount?.id === account.id
                  ? 'logging-in'
                  : ''
              }`}
              tabIndex="-1"
            ...
            >
              <AccountIcon>
                <img src={account.avatar} alt={account.name.toLowerCase()} />
              </AccountIcon>
              <AccountDetails>
                <AccountName>{account.name}</AccountName>
                {isLoggingIn && activeAccount?.id === account.id && (
                  <LoadingSettings className="loading-settings">
                ...
                {activeAccount?.id === account.id &&
                  account.hasPassword &&
                  !isLoggingIn && (
                    <PasswordSection className="login-screen__password">
                      <span>Parolanızı yazın</span>
                      <PasswordForm onSubmit={handlePasswordSubmit}>
                        ...
                        <QuestionButton
                          className="login-screen__question"
                          type="button"
                        >
                          <span>?</span>
                        </QuestionButton>
                      ...
                      {loginError && (
                        <ErrorText>Incorrect password. Try again.</ErrorText>
                      )}
                    </PasswordSection>
                  )}
              </AccountDetails>
